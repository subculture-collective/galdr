...
func TestCustomerList_QueryParams(t *testing.T) {
	...
	h := NewCustomerHandler(mock)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/customers?page=2&per_page=10&sort=mrr&order=desc&risk=high&churn_risk=high&search=acme&source=stripe", nil)
	req = req.WithContext(auth.WithOrgID(req.Context(), orgID))
	rr := httptest.NewRecorder()
