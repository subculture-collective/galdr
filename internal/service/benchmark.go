...
type BenchmarkComparisonAggregateReader interface {
...
type BenchmarkComparisonService struct {
	...
	aggregates    BenchmarkComparisonAggregateReader
}
...
type BenchmarkInsightAggregateReader interface {
	ListAllLatestAggregates(ctx context.Context) ([]repository.BenchmarkAggregate, error)
}
...
func (s *BenchmarkInsightNotificationService) RunOnce(ctx context.Context) error {
	...
	aggregates, err := s.aggregates.ListAllLatestAggregates(ctx)
	if err != nil {
